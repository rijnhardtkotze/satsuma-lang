...
**How do I represent Apache Avro with logical types and null unions?**
Use `(format avro)` and explicitly label primitives with `(avro_type bytes, logical_type decimal)`. Guide: [Apache Avro Conventions](docs/conventions-for-schema-formats/avro/conventions.md) | Example: [cobol-to-avro/pipeline.stm](examples/cobol-to-avro/pipeline.stm)
